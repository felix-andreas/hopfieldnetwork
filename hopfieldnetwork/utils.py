...
# plot network
def plot_network_development(network, timesteps, mode, exact_state, outputpath, anno_hamming=True):
    fig, axarr = plt.subplots(1, timesteps)
    fig.set_size_inches(4 * timesteps, 4)
    N_sqrt = int(np.sqrt(network.N))
    for t in range(timesteps):
        axarr[t].imshow(np.copy(network.S.reshape((N_sqrt, N_sqrt))), cmap='Blues', vmin=-1, vmax=+1, interpolation='none')
